...
function ProductImage({ item, featured = false }: { item: SelectedProduct; featured?: boolean }) {
  ...
  return (
    <div className={`relative overflow-hidden bg-[#eeeae2] ${featured ? "h-full min-h-[15rem]" : "min-h-0"}`}>
      {failed ? (
...
function ProductDetails({ item }: { item: SelectedProduct }) {
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 border-t border-[var(--line)] py-2 first:border-t-0">
      <div className="min-w-0">
        <h4 className="truncate text-sm font-semibold leading-snug">{item.product.name}</h4>
        <p className="mt-0.5 text-xs text-[var(--muted-ink)]">In stock · Size {item.selectedSize}</p>
      </div>
      <p className="text-sm font-semibold tabular-nums">{formatUsd(item.product.priceCents)}</p>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        <div className="grid min-h-[15rem] grid-cols-[1fr_0.36fr] gap-1.5 border-b border-[var(--line)] bg-[#e8e3da] p-1.5">
        ...
        <div className="flex min-w-0 flex-1 flex-col p-4">
          ...
          <p className="mt-2 line-clamp-2 text-xs leading-5 text-[var(--muted-ink)]" id={explanationId}>{outfit.explanation}</p>
          ...
          <div className="mt-auto flex items-end justify-between gap-3 border-t border-[var(--line)] pt-3">
            <div>
              ...
              <p className="mt-1 font-serif text-xl font-semibold tabular-nums">{formatUsd(outfit.totalCents)}</p>
            </div>
          ...
          <div className="mt-3" id={totalId}>
            <label className={`flex min-h-11 w-full items-center justify-between gap-3 bg-[var(--sage-dark)] px-4 font-bold text-white ${disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:bg-[var(--ink)]"}`} htmlFor={cardId}>
